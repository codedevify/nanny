...
const Booking = mongoose.model('Booking', new mongoose.Schema({
  ...
  day: String, time: String, service: String, paymentMethod: String,
  paymentId: String, status: { type: String, default: 'Pending' },
...
(async () => {
  ...
  if (config) {
    ...
    if (config.paypalClientId && config.paypalSecret) {
      paypal.configure({ mode: 'sandbox', client_id: config.paypalClientId, client_secret: config.paypalSecret });
    }
  }
  updateTransporter();
})();

// Dynamic Email Transporter
...
app.post('/api/payment/stripe', async (req, res) => {
  ...
  if (!adminConfig.stripeSecretKey) return res.status(400).json({ error: 'Stripe not configured' });
...
app.post('/api/payment/paypal', (req, res) => {
  const { amount } = req.body;
  if (!adminConfig.paypalClientId) return res.status(400).json({ error: 'PayPal not configured' });
  const create_payment_json = {
    intent: 'sale', payer: { payment_method: 'paypal' },
    redirect_urls: { return_url: `https://${req.headers.host}/success`, cancel_url: `https://${req.headers.host}/cancel` },
